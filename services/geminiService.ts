...
const callWithRetry = async <T>(fn: () => Promise<T>, retries = 3, delay = 2000): Promise<T> => {
  try {
    ...
    // Check for 429 (Resource Exhausted) or Quota related errors
    ...
    if (retries > 0) {
      // If rate limited, we MUST wait significantly longer to let the bucket refill.
      // 10 seconds is usually safe for the free tier burst reset.
      const waitTime = isRateLimit ? Math.max(delay, 10000) : delay;
      
      console.warn(`API Call failed ${isRateLimit ? '[RATE LIMIT]' : ''}, retrying in ${waitTime}ms... (${retries} attempts left). Error: ${error.message}`);
